...
Продолжаем тему установки arch. В этой статье рассмотрим доведение системы до презентабильного вида.
...
## Настройка сеетевого имени и localhost
...
Для настройки localhost открываем файл:
...
```
127.0.0.1 vlad.localdomain vlad
```
...
Вторым этапом ставим тайловый менеджер, меню запуска и окно входа:
...
Окно авторизации по умолчанию выглядит хоть и хорошо, после консоли, но недостаточно. Установим другую тему.
...
Для себя я выбрал эту [тему](https://github.com/horst3180/Arc-theme). Для настройки нам понадобится утилита `lxappearance` и сама тема. Через утилиту переключаем тему на новую и настраиваем другие параметры, по необходимости.
...
Для дальнейшей настройки тайлового менеджера можно воспользоваться [репозиторием](https://github.com/vlad-ku/i3conf).
